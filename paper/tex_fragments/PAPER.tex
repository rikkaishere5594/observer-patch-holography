...
\textbf{Proposition 6.9 (The number of generations is N\_g = 3).} Under (i) intrinsic CP violation in the quark sector, (ii) UV-completability of SU(2)\_L (asymptotic freedom at one loop), (iii) MaxEnt/refinement stability selecting minimal viable spectrum, and (iv) the color-type role requiring \(N_c \ge 3\), the generation number is
...
\item
  Use the derived carrier/product structure from Section 6.2, which forces an intrinsically complex color type and hence \(N_c \ge 3\).
\end{enumerate}
...
Since the color-type requirement forces \(N_c \ge 3\), we have \(N_c + 1 \ge 4\), so:
...
\item
  Under \(T_{\mathrm{ext}}\), this is a realized-branch derived result with the stated admissibility premises.
\end{itemize}
...
\textbf{Theorem 6.14 (The number of colors is N\_c = 3).} Under the gauge structure SU(N\_c) \ensuremath{\times} SU(2)\_L \ensuremath{\times} U(1)\_Y, with one left-handed quark doublet Q per color, one left-handed lepton doublet L per generation, and the realized generation count \(N_g = 3\) from Proposition 6.9, the global SU(2) anomaly (Witten, 1982) requires N\_c to be odd. Under MAR (lexicographic minimization of \(C(\mathfrak S)\), where \(N_c\) is the third component), this yields:
...
\item
  \textbf{Realized generation count}: \(N_g = 3\) from Proposition 6.9.
\item
  \textbf{MAR} (Selection Axiom): among allowed values, the third component of the complexity vector \(C(\mathfrak S)\) is minimized.
\end{enumerate}

\textbf{Proof.} Count total SU(2) doublets across all generations:

\begin{itemize}
\tightlist
\item
  Quark doublets: \(N_g N_c\) copies (one per color in each generation),
\item
  Lepton doublets: \(N_g\) copies (one per generation).
\end{itemize}
...
With \(N_g = 3\), Witten anomaly cancellation requires this to be even:

\[
3(N_c + 1) \equiv 0 \pmod{2} \quad \Rightarrow \quad N_c \text{ is odd}.
\]

The Witten constraint alone allows N\_c \ensuremath{\in} \{1, 3, 5, 7, ...\}. N\_c = 1 fails admissibility (SU(1) is trivial, no complex nonabelian charge type). MAR then selects N\_c = 3 as the smallest nontrivial value. QED.

\textbf{Status.} With the realized generation count \(N_g=3\), the Witten anomaly derives \(N_c\) odd. The specific value \(N_c = 3\) then follows from MAR applied to the realized admissible class. Under the extended theory \(T_{\mathrm{ext}}\), this is derived on the realized branch with the stated premises.
