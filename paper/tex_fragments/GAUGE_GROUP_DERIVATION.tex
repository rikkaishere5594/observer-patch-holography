...
\textbf{Proposition 5.8 (Z\textsubscript{6} quotient from hypercharge quantization).} If the realized matter spectrum has hypercharges quantized in sixths under the electroweak normalization convention \(Q=T_3+Y\) with \(Q(\nu_L)=0\), then the subgroup of \(SU(3) \times SU(2) \times U(1)\) acting trivially on all physical states is exactly \(\mathbb{Z}_6\).

\emph{Proof.} The SM matter content with hypercharge assignments \((Q_L, u_R, d_R, L_L, e_R, H)\) has hypercharges \(Y \in \{1/6, 2/3, -1/3, -1/2, -1, 1/2\}\), all in \(\frac{1}{6}\mathbb{Z}\). Write \(q:=6Y\in\mathbb Z\) and \(\eta:=e^{i\pi/3}\), so the \(U(1)\) factor acts by \(\eta^q\) on charge-\(q\) multiplets. The center of \(SU(3) \times SU(2) \times U(1)\) is \(\mathbb{Z}_3 \times \mathbb{Z}_2 \times U(1)\). The subgroup acting trivially on all realized representations is generated by

\[(\omega_3, -1, e^{i\pi/3}) \in SU(3) \times SU(2) \times U(1),\]

where \(\omega_3 = e^{2\pi i/3}\) is the \(\mathbb{Z}_3\) center of \(SU(3)\). Writing \(g_0:=(\omega_3,-1,\eta)\), one checks
...
Thus \(g_0\) acts trivially on all realized multiplets, and its powers generate a subgroup of order six. Conversely, let \((\omega_3^a,(-1)^b,e^{i\theta})\) be a central element acting trivially on all realized states. Triviality on \(e_R\) forces \(e^{i\theta}=\eta^n\) for some \(n\in\mathbb Z/6\mathbb Z\). Triviality on \(Q_L\) then gives

\[
\omega_3^a(-1)^b\eta^n=1,
\]

so \(a\equiv n \pmod 3\) and \(b\equiv n \pmod 2\). Hence every trivial central element is \(g_0^n\). Therefore the full kernel is exactly \(\mathbb{Z}_6\), and

\[G_{\mathrm{phys}} = \frac{SU(3) \times SU(2) \times U(1)}{\mathbb{Z}_6}.\]

Equivalently, \(G_{\mathrm{phys}} \cong S(U(3) \times U(2))\).

The hypercharge quantization itself follows from anomaly cancellation (condition ii) with the minimal Yukawa-complete spectrum (condition iv), and the electroweak normalization convention \(Q=T_3+Y\) with \(Q(\nu_L)=0\) fixes the standard sixth-integer lattice. Theorem 6.13 of Part I of this manuscript derives the hypercharge assignments from these conditions for \(N_c = 3\), yielding precisely that lattice. This is Proposition 6.6 of Part I of this manuscript. \(\square\)
